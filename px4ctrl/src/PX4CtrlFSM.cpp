...
void PX4CtrlFSM::process() {
  ...
  case MANUAL_CTRL: {
                               ...
                               // 这个变量是怎么赋值的？根据通道的信息来确定吗？
    {
      ...
      {
              ROS_ERROR("[px4ctrl] Reject AUTO_HOVER(L2). You are sending
      commands before toggling into AUTO_HOVER, which is not allowed. Stop
      sending commands now!"); break;
      }*/
  ...
  case AUTO_HOVER: {
    if (!rc_data.is_hover_mode || !odom_is_received(now_time)) {
      ...
      ROS_WARN("[px4ctrl] AUTO_HOVER(L2) --> MANUAL_CTRL(L1)");
               ...
               loss_target_time_count == 0 /*&& !takeoff_land_data.triggered*/
               && !in_landing) {
                                   ...
                                   // 用遥控器来决定无人机的悬停位置和偏航角
{
  ...
  hover_pose(2) += rc_data.ch[2] * param.max_manual_vel * delta_t *
                   (param.rc_reverse.throttle ? -1 : 1); // 通道2是油门？
