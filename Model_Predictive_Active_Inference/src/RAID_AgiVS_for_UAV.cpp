...
RAID_AgiVS::RAID_AgiVS(ros::NodeHandle &nh)
    : nh(nh), px4_state(3), x_real(0.0), xv_real(0.0), y_real(0.0),
      yv_real(0.0), z_real(0.0), zv_real(0.0), des_yaw(0.0),
      optimizer_xyz(dt, Np, precice_z1, precice_z2, precice_w1, precice_w2,
                    precice_z_u, e1, umin, umax) {
  ...
  px4_state_sub = nh.subscribe("/px4_state_pub", 1, &RAID_AgiVS::StateCallback,
                               this, ros::TransportHints().tcpNoDelay());
  relative_pos_sub =
      nh.subscribe("/hat_error_xyz", 1, &RAID_AgiVS::relative_pos_Callback,
                   this, ros::TransportHints().tcpNoDelay());
...
void RAID_AgiVS::xyzAxisControlLoop() {
  ...
  while (ros::ok()) {
    if (px4_state == 3) {
      ...
      optical_xyz = optimizer_xyz.optimize(xyz_real, xyz_v_real, optical_mu,
                                           optical_mu_p);
      auto end_time = std::chrono::high_resolution_clock::now();
      double elapsed_time =
          std::chrono::duration<double>(end_time - start_time).count();
      ROS_INFO("Execution time x: %f seconds", elapsed_time);
      // cout<<"optical_xyz: "<<optical_xyz[0]<<" "<<optical_xyz[1]<<"
      // "<<optical_xyz[2]<<endl;
      cout << xyz_real << endl;
...
void RAID_AgiVS::adjustBias(
    bool use_bias) // which_axis: 0 for x, 1 for y, 2 for z
{
...
void RAID_AgiVS::relative_pos_Callback(
  ...
  if (land_or_just_tracking) {
    if (abs(msg->data[6]) > 0.15 || abs(msg->data[7]) > 0.15) {
      ...
      if ((abs(msg->data[8]) < 0.15 && msg->data[9] == 0 && px4_state == 3) ||
          keep_in_land) {
        // Send forced landing command as we might be losing sight of the
        // target, open-loop close proximity landing
        keep_in_land = true;
        std_msgs::Bool land_msg;
        land_msg.data = true;
        pub_land.publish(land_msg);
